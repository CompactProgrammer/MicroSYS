...
# MicroSYS necessary files
nasm 'source/boot.asm' -f bin -o 'build/boot.bin';

# Combine binaries
cat 'build/sysarea.bin' 'build/voldesc.bin' 'build/bootcat.bin' 'build/lptab.bin' 'build/mptab.bin' 'build/rootdir.bin' 'build/boot.bin' > 'images/disk1.iso';
